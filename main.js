...
const exercise2 = (min, max) => {
    ...
    // log range with evenNum
    console.log(`Even numbers from ${min} - ${max}: ${[...evenNum]}`);
};
...
const compare = (num1, num2) => {
    ...
    if (num1 > num2) return `${num1} is the bigger number`;
...
const exercise5 = () => {
    ...
    let balance = parseFloat(
        prompt(`Exercise 5:\nPlease enter your current balance in the box below.`)
    );
    ...
    while (continueTransactions) {
        ...
        // if user wants to deposit, ask for deposit amount then add to balance
            ...
            let deposit = parseFloat(
                prompt("Exercise 5:\nPlease enter the amount of money you would like to deposit in the box below.")
            );
            balance += deposit;
            // if user wants to withdraw, ask for withdraw amount then subtract from balance
        } else if (action.toUpperCase() === "WITHDRAW") {
            let deposit = parseFloat(
                prompt("Exercise 5:\nPlease enter the amount of money you would like to withdraw in the box below.")
            );
            balance -= deposit;
